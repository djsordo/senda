...
export class PorteriaComponent implements OnInit, AfterViewInit {
  ...
  equis1: ElementRef;
  public porteriaStyle = { position: 'relative', top: 0, left: 0 };
  ...
  public onPorteriaClick(event) {
    if (event.target.id === 'porteria') {
      this.adaptaAreas();
      this.equis1Style.left = `${
        event.offsetX - this.equis1.nativeElement.width / 2
      }px`;
      this.equis1Style.top = `${
        event.offsetY - this.equis1.nativeElement.height / 2
      }px`;
      this.equis1Style.visibility = 'visible';
      console.log(
        `el area se llama ${this.getAreaNameOfPoint([
          event.offsetX,
          event.offsetY,
        ])}`
      );
  ...
  public isPointInsidePolygon(
    ...
    const INFINITE = 10000; /* use of MAX_INT can cause overflow */
    const extreme = [INFINITE, point[Y]];
    let intersections = 0;
    let i = 0;
    do {
      ...
      if (this.doIntersect(polygon[i], polygon[next], point, extreme)) {
        if (
          this.orientation(polygon[i], point, polygon[next]) ===
          0 /* collinear */
        ) {
   ...
   */
  private adaptaAreas() {
    const actualWidth = this.porteria.nativeElement.width;
    const actualHeight = this.porteria.nativeElement.height;
    for (const area of this.areas) {
      area.polygonAdapted = [];
      for (const point of area.polygon) {
        const x = point[X] * actualWidth;
